...
interface IGamesStore{
	...
	//function change...Rate(uint32 _...Rate) public;  token name?
	function addLicence(uint32 _gameId,uint _amount) public;
}


contract SmartOakCompany is SmartOakContract {
    ...
    constructor(
        ...
        SmartOakToken token = SmartOakToken(reg.getAddress("SmartOakToken"));
        ...
        reg.setAddress("SmartOakCompany", address(this));
    ...
    /*   (change ... for token name) function change...Rate(uint256 _price) public {
		if(guardSumAndCaller(0,false)){
			IGamesStore store = IGamesStore(reg.getAddress("GamesStore"));
			store.change...Rate(uint32(_price)); */
		}
    }   
    ...
    function updateCompany(address _to) public{ 
		if(guardSumAndCaller(0,true)){
			...
			reg.setAddress("SmartOakCompany",_to);
		}
    }
}